/**
 * UAT: `allw-hook --version` reports the version from the package's OWN package.json — never a
...
function runCli(args, cliPath = CLI) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error("allw-hook --version did not exit within 5000ms"));
    }, 5000);
    let stdout = "";
    let stderr = "";
...
test("allw-hook --version prints the package.json version and exits 0", async () => {
  const { code, stdout } = await runCli(["--version"]);
  assert.equal(code, 0);
  assert.equal(stdout.trim(), expectedVersion);
});

test("allw-hook -v is an alias for --version", async () => {
...
test("the reported version is a real semver-shaped string, not a placeholder", () => {
  // Negative guard: a re-hardcoded "0.0.0" / "0.0.1" placeholder (the classic drift bug) would slip
  // past an equality check if package.json were also that value, so assert the version is non-empty
  // and looks like a release. (package.json is the source of truth; this only rejects placeholders.)
  assert.match(expectedVersion, /^\d+\.\d+\.\d+/);
  assert.notEqual(expectedVersion, "0.0.0");
});

// Regression: the `allw-hook` bin is invoked through a symlink in `node_modules/.bin`, so
...
// runs and the bin silently prints nothing (caught first by scripts/smoke-packaged-install.mjs).
test("allw-hook --version works when invoked through a symlink (node_modules/.bin parity)", async () => {
  const dir = mkdtempSync(join(tmpdir(), "allw-hook-symlink-"));
  const link = join(dir, "allw-hook");
  try {
