...
export const KEYFILE_VERSION = 2 as const;
const LEGACY_KEYFILE_VERSION = 1 as const;
...
export function validateKeyfile(value: unknown): Keyfile {
  ...
  if (value.version !== KEYFILE_VERSION && value.version !== LEGACY_KEYFILE_VERSION) {
    throw new Error(
  ...
  // Every required field is a validated 32-byte key and optional fields have been shape-checked.
  return { ...value, version: KEYFILE_VERSION } as unknown as Keyfile;
}
