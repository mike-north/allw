...
| Requester impersonator         | Actor identity shown to human                                        | Actor-key attestation is verified on device before the human approves; the actor key is resolved from root-signed account state (#16), so a relay-substituted key cannot forge a verified origin. | A device with no root-signed account state shows ⚠ UNVERIFIED (fail-closed) until relay distribution of account state is wired (#104); owner: account-state distribution + warning UX. |
