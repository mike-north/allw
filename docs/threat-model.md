...
| OpenClaw bridge holds `operator.admin` in trusted-deployment mode           | OpenClaw's approval delivery is filtered per record, not scope-only: on `operator.approvals` alone a generic paired bridge is excluded from the events and lists for every requester-bound approval, so it cannot discover the requests it exists to gate. Admin is the only currently supported way to see them — it is upstream's own operator-CLI pattern, and the default pairing handoff profile already grants it. The delta: admin satisfies every `operator.*` scope, so a **compromised** bridge could rewrite the exec policy it gates (lower `tools.exec.mode`, edit the allowlist, add standing grants) and remove the need to ask a human at all. On least-privilege scopes the worst case is bounded at denying, or approving requests it was already shown. | Admin is **explicit opt-in only**, never inferred or defaulted, and the mode is logged at startup and recorded in the audit chain for every verdict. Mitigations: self-hosted gateway (admin is authority over the operator's own gateway, not third-party infrastructure), one revocable paired device identity rather than a shared secret, and after-the-fact detection of config mutation from the bridge's device id. The least-privilege mode is specified and fails closed/unavailable today; it becomes active unchanged once upstream ships reviewer designation. `docs/openclaw-integration.md` §4.1, `docs/hitl-seam.md` §11.1, `docs/upstream-asks/openclaw-reviewer-designation.md`. |
