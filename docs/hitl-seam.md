...
#### OpenClaw compatibility finding

The current OpenClaw public external-client seam is not sufficient for the bridge described in
[openclaw-integration.md](./openclaw-integration.md). `operator.approvals` is necessary but record
visibility is additionally filtered by requester identity, connection/client binding, explicit
`approvalReviewerDeviceIds`, or trusted internal runtime status. A newly paired generic bridge is
neither the requester nor a designated reviewer, so it does not receive ordinary pending events—the
only public carrier of the full request and `systemRunPlan`. Unified `approval.get` is sanitized and
cannot reconstruct that plan. Scope-only subscribe/list/backfill/reconcile therefore cannot provide
the required exact-request binding on OpenClaw 2026.8.1. This is an upstream integration blocker,
not a reason to weaken L0. ([record visibility](https://github.com/openclaw/openclaw/blob/1d526c5c0ef635b4b7fda952c2b26da0c0290652/src/gateway/server-methods/approval-record-lookup.ts), [approval sharing](https://github.com/openclaw/openclaw/blob/1d526c5c0ef635b4b7fda952c2b26da0c0290652/src/gateway/server-methods/approval-shared.ts), [request context](https://github.com/openclaw/openclaw/blob/1d526c5c0ef635b4b7fda952c2b26da0c0290652/src/gateway/server-request-context.ts))
...
| OpenClaw's current external projection omits the canonical execution plan                                                                     | The adapter fails closed or remains unavailable until an upstream reviewer binding exposes the exact plan; the common contract does not weaken its request identity to accommodate a lossy host seam. |
