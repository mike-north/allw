...
5. **Plan-divergence containment.** After approving an exec request, attempt to change the
   command/cwd before the forwarded `system.run` and confirm the gateway still forwards the stored
   approved plan. Separately supply a mismatched approval plan to the node host and confirm it
   rejects the run. This proves the canonicalization and final enforcement bindings compose.
...
Plan divergence: PASS/FAIL - gateway retained the stored plan; node host rejected a mismatched plan
