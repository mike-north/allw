 ...
 * line is `*`. npm honors a nested `.gitignore` when assembling a tarball: even with `vendor` on the
 * SDK's `files` allowlist, that `*` would exclude the `.wasm` + glue from the published package,
 * defeating the whole point of vendoring (an `npm install` would then need a Rust toolchain — a
 * violation of `docs/architecture.md`: the wasm glue must SHIP in the package).
 *
 ...
 *
 * @see ../packages/sdk/package.json (`files` includes `vendor`; `build:wasm` chains this script)
